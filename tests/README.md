...
Known limitations (flagged during the 2026-08 audit, not fixed):
...
2026-08-09 after verifying that no published RSZB or LMFDB labels depend on
the changed outputs; `test_gl2base.m` now brute-forces all level-4 classes,
the only affected modulus — Magma's element ordering over Z/4 is the sole
place GrpMatElt order and Eltseq order disagree.)
